...
from emulator.models.swin_layers import swintransformerblock
...
class CircConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, bias=True):
        super().__init__()
        if kernel_size != 3:
            raise ValueError("CircConv2d currently supports kernel_size=3 only")
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=3,
...
class EmulatorEnsemble(nn.Module):
